...
export default function VideoPlayer({ roomId, movie, isWatchParty, user }) {
  ...
  // ⭐️ MARK 1: State สำหรับ Local Clock
  ...
  function syncToState(state) {
    ...
    // ⭐️ MARK 2: อัปเดต localTime เมื่อ Sync จาก Server
    setLocalTime(predictedTime); 
    ...
    if (state.playing) {
      ...
      if (p && p.catch) {
        p.catch(() => {
          ...
          stopLocalClock(); // หยุด Local Clock หาก Autoplay ถูกบล็อก
        });
      } else {
          setIsAutoplayBlocked(false);
          startLocalClock(); // ⭐️ เริ่ม Local Clock เมื่อเล่นได้สำเร็จ
      }
    } else {
        v.pause();
        stopLocalClock(); // ⭐️ หยุด Local Clock เมื่อหยุดเล่น
    }
  }
  
    // ---------------------- Local Clock Functions ----------------------

    // ⭐️ MARK 3: ฟังก์ชันควบคุม Local Clock
  ...
  useEffect(() => {
    ...
    if (isWatchParty) {
      ...
      socket.on("room_state", (state) => {
        ...
        syncToState(state); // syncToState จะจัดการการเริ่ม/หยุด Local Clock
        ...
        if (!isLoggedIn) {
            if (isAutoplayBlocked) {
                if (action === "play") {
                    video.play();
                    setIsAutoplayBlocked(false); 
                    startLocalClock(); // ⭐️ MARK 4: เริ่ม Local Clock เมื่อปลดบล็อก
                } else if (action === "pause") {
                    video.pause();
                    stopLocalClock(); // ⭐️ MARK 5: หยุด Local Clock เมื่อหยุด
                }
            }
            return; 
        }
      ...
      const onPlay = () => {
        ...
        startHeartbeat();
        startLocalClock(); // ⭐️ MARK 6: เริ่ม Local Clock เมื่อ Play
      };
      const onPause = () => {
        emitAction("pause", video.currentTime);
        setStatus((s) => ({ ...s, playing: false }));
        stopHeartbeat();
        stopLocalClock(); // ⭐️ MARK 7: หยุด Local Clock เมื่อ Pause
      };
      const onSeeked = () => {
      ...
      return () => {
        ...
        if (seekTimeout.current) clearTimeout(seekTimeout.current);
        stopHeartbeat();
        stopLocalClock(); // ⭐️ MARK 8: Cleanup Local Clock
      };
    ...
    // ------------------ Single Private Mode ------------------
      ...
      // ⭐️ MARK 9: Event Listener สำหรับโหมด Private
      const onTimeUpdatePrivate = () => setLocalTime(video.currentTime);
      const onPlayPrivate = () => startLocalClock();
      const onPausePrivate = () => stopLocalClock();
      
      video.addEventListener("timeupdate", onTimeUpdatePrivate); // ใช้อีเวนต์ timeupdate เพื่อความแม่นยำในโหมด Private
      ...
      return () => {
          stopLocalClock();
          video.removeEventListener("timeupdate", onTimeUpdatePrivate);
  ...
  }, [roomId, movie.id, isWatchParty, user, isAutoplayBlocked]); 
  ...
  return (
    <div className="w-full h-full flex flex-col bg-black">
      ...
      <video
        ref={videoRef}
        src={movie.videoUrl}
        // ⭐️ MARK 10: เงื่อนไข Controls: ถ้าเป็น Watch Party และไม่มี user หรือ Autoplay ถูกบล็อก จะเปิด Controls
...
//       // ⭐️ [FIX]: ปรับปรุง onSeeked โดยใช้ Debouncing และจัดการ canplay Listener
//       const onSeeked = () => {
//         const t = video.currentTime;
        
//         // 1. Clear timeout เก่าเสมอเมื่อมีการ Seeked ใหม่เข้ามา (Debouncing)
//         if (seekTimeout.current) clearTimeout(seekTimeout.current);

//         // 2. สร้างฟังก์ชันสำหรับการส่ง Seek Action
//         const sendSeekAction = (time) => {
//             // เช็คว่าวิดีโอพร้อมเล่นในระดับที่พอจะ Seek ได้หรือไม่
//             if (video.readyState < 3) {
//                 // ถ้ายังไม่พร้อม ให้รอ Event 'loadeddata' หรือ 'canplay' 
//                 // ใช้ 'loadeddata' เพื่อความมั่นใจว่ามีข้อมูลพอที่จะ Seek
...
//         // 3. ใช้ Debouncing: หน่วงเวลา 200ms เพื่อรอให้ผู้ใช้หยุด Seek ชั่วคราว
//         seekTimeout.current = setTimeout(() => {
//             sendSeekAction(t);
//         }, 200); 
//       };
...
//         socket.off("room_state");
//         socket.off("player_action");
//         if (seekTimeout.current) clearTimeout(seekTimeout.current); // ⭐️ สำคัญ: เคลียร์ Timeout ใน Cleanup
//         stopHeartbeat();
//         stopLocalClock(); // ⭐️ MARK 8: Cleanup Local Clock
//       };
